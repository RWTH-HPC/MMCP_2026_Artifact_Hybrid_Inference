...
def plotC(real_speeds, expected_speeds, order):
    ...
    plt.bar(x-width/2, [expected_speeds.get(c, np.nan) for c in ordered_cases], width=width, label="Theoretical Speedup")
    plt.bar(x+width/2, [real_speeds.get(c, np.nan) for c in ordered_cases], width=width, label="Real Speedup")
    #plt.xticks(x, ordered_cases, rotation=45, ha="right")

    plt.xlabel("Case (Nodes - GPUs)", fontsize=10)
    plt.ylabel("Speedup", fontsize=10)
    plt.title("Theoretical vs Real Optimal Speedup per Case", fontsize=12)
    x_labels = []
    ...
    plt.xticks(x, x_labels, fontsize=10)# plt.xticks(rotation=45, ha="right")
    plt.grid(True, linestyle="--", alpha=0.6)
    plt.legend(loc="upper right", fontsize=11)
    plt.tight_layout(pad=0.8)
    plt.savefig("summary_real_vs_expected_speedup.pdf", bbox_inches="tight")
    plt.close()
...
def plotD(all_runtimes):
    ...
    for ax_idx, (gpu_label, cases) in enumerate(gpu_groups.items()):
        ...
        ax.set_xlabel("CPU Fraction", fontsize=10)
        if ax_idx == 0:
            ax.set_ylabel("Speedup", fontsize=10)
        ax.set_title(f"{gpu_label[:1]} {gpu_label[1:4]}"+("s" if gpu_label[:1] == "4" else ""))
    ...
    fig.suptitle("Hybrid Inference Scalability", fontsize=12, y=0.92)
    fig.legend(cpus, loc="lower center", ncol=4,bbox_to_anchor=(0.5, -0.19), fontsize=11, title="Nodes")
    fig.tight_layout(rect=[0, 0, 1, 0.96], pad=0.8)
    #plt.subplots_adjust(bottom=0.25)  
    plt.savefig("summary_speedup_vs_worksplit.pdf", bbox_inches="tight")
    plt.close()
    print("Saved plotD")
...
def plotE(real_speeds, expected_speeds, expected_opt_splits, real_opt_splits, order):
    ...
    plt.xlabel("Case (Nodes - GPUs)", fontsize=10)
    plt.ylabel("CPU Fraction", fontsize=10)
    x_labels = []
    for name in ordered_cases:
        # Find where the "Nodes" and "GPU" parts are based on capitalization
        # Assume format is like "4CPUNodes2GPU"
    ...
    #plt.xticks(x, ordered_cases, rotation=45, ha="right")
    plt.title("Theoretical vs Real Optimal CPU Fraction per Case", fontsize=12)
    #plt.tick_params(axis='x', rotation=45)
    plt.grid(True, linestyle="--", alpha=0.5)
    plt.legend(loc="upper right", fontsize=11)
    plt.tight_layout(pad=0.8)
    plt.savefig("summary_expected_vs_real_worksplit.pdf", bbox_inches="tight")
    plt.close()
...
def plotF(expected_speeds, expected_opt_splits, real_opt_splits, all_case_runtimes, order):
    ...
    plt.xlabel("Case (Nodes - GPUs)", fontsize=10)
    plt.ylabel("Speedup", fontsize=10)
    plt.title("Comparison of Theoretical and Real Speedup per Case", fontsize=12)
    plt.xticks(x, ordered_cases, rotation=45, ha="right")
    plt.grid(True, linestyle="--", alpha=0.5)
    plt.legend(loc="upper right", fontsize=11)
    plt.tight_layout(pad=0.8)
    plt.savefig("summary_theoretical_vs_real_speedup.pdf", bbox_inches="tight")
    plt.close()
...
def plotH(real_opt_splits, order):
    ...
    plt.xlabel("Case (Nodes - GPUs)",fontsize=10)
    plt.ylabel("Total Energy (Wh)",fontsize=10)
    plt.title("Energy Consumption per Case and Worksplit",fontsize=12)
    x_labels = []
...
def plot_parabel2_scaled_combined(all_case_runtimes,
                                  expected_opt_splits=None,
                                  real_opt_splits=None,
                                  output_dir="."):
    """
    ...
    Both plots include:
        • Theoretical CPU runtime line
        • Theoretical GPU runtime line
        • Real CPU inference (if available)
        • Real GPU inference (if available)
        • Vertical theoretical optimum line
        • Vertical real optimum line

    With:
        • Shared y-label
        • Shared legend across subplots
        • Fontsizes and spacing consistent with existing multi-subplot patterns
    """

    import os, re, numpy as np
    import matplotlib.pyplot as plt

    example_cases = ["1CPUNodes1GPU", "4CPUNodes1GPU"]

    # Prepare figure layout (matching your conventions)
    ...
    # ---- helper function reused internally -----------------------------

    def parse_real_inference(exp_dir):
        real_cpu_inf = {}
        real_gpu_inf = {}
        ...
        return real_cpu_inf, real_gpu_inf

    # ---------------------------------------------------------------------

    for ax, example_case in zip(axes, example_cases):

        if example_case not in all_case_runtimes:
            print(f"Case '{example_case}' not found!")
            ax.set_visible(False)
            continue

        runtimes = all_case_runtimes[example_case]

        # parse worksplit keys
        worksplits = sorted([
            float(k.replace(",", "."))
        ...
        if not worksplits:
            print(f"No valid worksplits in {example_case}")
            ax.set_visible(False)
            continue

        # ------------------ Real inference extraction ---------------------
        exp_dir = os.path.join(example_case, "Experiments")
        if not os.path.exists(exp_dir):
            print(f"No experiment directory for {example_case}")
        real_cpu_inf, real_gpu_inf = parse_real_inference(exp_dir)

        # ------------------ Theoretical CPU/GPU times ---------------------
        i_node = example_case.find("CPU")
        i_gpu = example_case.find("GPU")
        n_nodes = int(example_case[:i_node])
        n_gpus = int(example_case[i_node + 8:i_gpu])

        cpu_time = runtimes.get("1,00", max(runtimes.values())) * \
                   (n_nodes * 96) / (n_nodes * 96 - n_gpus)
        gpu_time = runtimes.get("0,00", min(runtimes.values()))
        ...
        (cpu_th_line,) = ax.plot(
            worksplits, cpu_line, label="Theoretical CPU Runtime"
        )
        (gpu_th_line,) = ax.plot(
            worksplits, gpu_line, label="Theoretical GPU Runtime"
        )

        # Store legend handles
        for name, handle in [
            ("Theoretical CPU Runtime", cpu_th_line),
            ("Theoretical GPU Runtime", gpu_th_line),
        ]:
            if name not in legend_handles:
                legend_handles[name] = handle
                legend_labels[name] = name

        # ------------------ Real CPU/GPU inference curves -----------------
        if real_cpu_inf:
            xs = sorted([float(k.replace(",", ".")) for k in real_cpu_inf])
            ys = [real_cpu_inf[f"{x}".replace(".", ",")] for x in xs]
            (rci,) = ax.plot(xs, ys, "o--", label="Real CPU Inference", color="tab:orange")
            if "Real CPU Inference" not in legend_handles:
                legend_handles["Real CPU Inference"] = rci
                legend_labels["Real CPU Inference"] = "Real CPU Inference"

        if real_gpu_inf:
            xs = sorted([float(k.replace(",", ".")) for k in real_gpu_inf])
            ys = [real_gpu_inf[f"{x}".replace(".", ",")] for x in xs]
            (rgi,) = ax.plot(xs, ys, "o--", label="Real GPU Inference", color="tab:green")
            if "Real GPU Inference" not in legend_handles:
                legend_handles["Real GPU Inference"] = rgi
                legend_labels["Real GPU Inference"] = "Real GPU Inference"

        # ------------------ Optimal vertical lines -------------------------
        # theoretical
        if expected_opt_splits and example_case in expected_opt_splits:
            try:
                x = float(str(expected_opt_splits[example_case]).replace(",", "."))
                line = ax.axvline(x, color="tab:green", linestyle="--", linewidth=1.2)
                if "Theoretical Opt" not in legend_handles:
                    legend_handles["Theoretical Opt"] = line
                    legend_labels["Theoretical Opt"] = "Theoretical Opt"
            except:
                pass
        ...
        if real_opt_splits and example_case in real_opt_splits:
            try:
                x = float(str(real_opt_splits[example_case]).replace(",", "."))
                line = ax.axvline(x, color="crimson", linestyle=":", linewidth=1.2)
                if "Real Opt" not in legend_handles:
                    legend_handles["Real Opt"] = line
                    legend_labels["Real Opt"] = "Real Opt"
            except:
                pass

        # ------------------ Formatting -------------------------
        ax.set_title(example_case, fontsize=11)
        ax.grid(True, linestyle="--", alpha=0.6)
        ax.tick_params(axis='both', labelsize=10)
        ax.set_xlabel("CPU Fraction", fontsize=10)

    # Shared y-label
    axes[0].set_ylabel("Runtime (s)", fontsize=10)
    ...
    fig.suptitle("Scaled Theoretical Runtime Model", fontsize=12, y=0.98)
    fig.tight_layout(rect=[0, 0.02, 1, 0.94], pad=0.8)

    out_path = os.path.join(output_dir, "combined_parabel2_scaled.pdf")
...
def main():
    ...
    plot_parabel2_scaled_combined(all_case_runtimes, expected_opt, real_opt_splits)
