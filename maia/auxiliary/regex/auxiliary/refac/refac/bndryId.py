# Script to remove all accesses to the m_bndryId member variable of the
# cell types inside ZFS.
#
# The following cases are identified (see also matchers below):
# - m_cells->a[ ... ].m_bndryId
# - cells[ ... ].m_bndryId
# - cpu_cells->a[ ... ].m_bndryId
#
...
# Rewrites occurrences inside blocks to a_boundaryId(...)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['bnd']:
        if "block" in match['begin_delimiter']:
            return "m_block->a_boundaryId(" + occurrence['delimited_str'] + ")"
        else: 
            return "m_block->a_boundaryId(" + occurrence['delimited_str'] + ")"
    elif ftype['partcont'] or ftype['particle']:
        return "blockPtr->a_boundaryId(" + occurrence['delimited_str'] + ") "
    else:
        return "a_boundaryId(" + occurrence['delimited_str'] + ") "


#def rewrite_gcell_inplace(occurrence, match, ftype):
#    return "globalGId(" + occurrence['delimited_str'] + ")"

# Rewrites ocurrences of custom collectors to spongeFactor(collector, ...)
#def rewrite_custom_collector(occurrence, match, ftype):
#    if ftype['grid'] or ftype['avg']:
#        if "cpu_cells" in match['begin_delimiter']:
#            return "boundaryId(cpu_cells, " + occurrence['delimited_str'] + ")"
#        elif "input_cells" in match['begin_delimiter']:
#            return "boundaryId(input_cells, " + occurrence['delimited_str'] + ")"
#        else:
#            exit("unknown pattern")
#    else:
#        if "cellsInput" in match['begin_delimiter']:
#            return "grid().spongeFactor(cellsInput, " + occurrence['delimited_str'] + ")"
#        else:
#            exit("unknown pattern")
...
end_pId_re = "\]\s*\.m_bndryId\\b"
end_pId_ggp_re = "\]\s*\.m_bndryId\)"
matchers = [
...
tokens = [
            ...
            # {'pattern': r"\bbndryId\b",
            #  'replacement': "bndryId_",
            #  'ftype': ['grid'],
            # }
         ]
