# Script to remove all accesses to the m_globalId member variable of the
# cell types inside ZFS.
#
# The following cases are identified (see also matchers below):
# - m_cells->a[ ... ].m_globalId
# - cells[ ... ].m_globalId
# - inside_cells->a[ ... ].m_globalId
# - cpu_cells->a[ ... ].m_globalId
#
...
# Rewrites occurrences inside grid to a_globalId(...), and inside blocks to
# grid().a_globalId(...)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg'] or ftype['cell']:
        return "a_globalId(" + occurrence['delimited_str'] + ")"
    else:
        return "grid().a_globalId(" + occurrence['delimited_str'] + ")"

def rewrite_gcell_inplace(occurrence, match, ftype):
    return "a_globalGId(" + occurrence['delimited_str'] + ")"

# Rewrites ocurrences of custom collectors to a_globalId(collector, ...)
def rewrite_custom_collector(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        if "cpu_cells" in match['begin_delimiter']:
            return "a_globalId(cpu_cells, " + occurrence['delimited_str'] + ")"
        elif "input_cells" in match['begin_delimiter']:
            return "a_globalId(input_cells, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
    else:
        if "cellsInput" in match['begin_delimiter']:
            return "grid().a_globalId(cellsInput, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
...
end_pId_re = "\]\s*\.m_globalId"
end_pId_ggp_re = "\]\s*\.m_globalId\)"
matchers = [
