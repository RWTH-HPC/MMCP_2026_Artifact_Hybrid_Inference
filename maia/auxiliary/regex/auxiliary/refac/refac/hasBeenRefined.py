# Script to remove all accesses to the m_hasBeenRefined member variable of the
# cell types inside ZFS.
#
# The following cases are identified (see also matchers below):
# - m_cells->a[ ... ].m_hasBeenRefined
# - cells[ ... ].m_hasBeenRefined
# - inside_cells->a[ ... ].m_hasBeenRefined
# - cpu_cells->a[ ... ].m_hasBeenRefined
#
...
# Rewrites occurrences inside grid to a_hasBeenRefined(...), and inside blocks to
# a_hasBeenRefined(...)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        return "a_hasBeenRefined(" + occurrence['delimited_str'] + ")"
    else:
        return "a_hasBeenRefined(" + occurrence['delimited_str'] + ")"

def rewrite_gcell_inplace(occurrence, match, ftype):
    return "a_hasBeenRefinedG(" + occurrence['delimited_str'] + ")"

# Rewrites ocurrences of custom collectors to a_hasBeenRefined(collector, ...)
def rewrite_custom_collector(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        if "cpu_cells" in match['begin_delimiter']:
            return "a_hasBeenRefined(cpu_cells, " + occurrence['delimited_str'] + ")"
        elif "input_cells" in match['begin_delimiter']:
            return "a_hasBeenRefined(input_cells, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
    else:
        if "cellsInput" in match['begin_delimiter']:
            return "a_hasBeenRefined(cellsInput, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
...
end_pId_re = "\]\s*\.m_hasBeenRefined\\b"
end_pId_ggp_re = "\]\s*\.m_hasBeenRefined\)"
matchers = [
    # matches m_cells->a
...
tokens = [
            # # matches hasBeenRefined in grid files (shadowing)
            # {'pattern': r"\bhasBeenRefined\b",
            #  'replacement': "hasBeenRefined_",
            #  'ftype': ['grid'],
            # }
         ]
