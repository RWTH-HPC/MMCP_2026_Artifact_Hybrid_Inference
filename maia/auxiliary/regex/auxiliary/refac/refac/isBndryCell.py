# Script to remove all accesses to the m_isBndryCell member variable of the
# cell types inside ZFS.
#
# The following cases are identified (see also matchers below):
# - m_cells->a[ ... ].m_isBndryCell
# - cells[ ... ].m_isBndryCell
# - inside_cells->a[ ... ].m_isBndryCell
# - cpu_cells->a[ ... ].m_isBndryCell
#
...
# Rewrites occurrences inside grid to a_isBndryCell(...), and inside blocks to
# a_isBndryCell(...)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        return "a_isBndryCell(" + occurrence['delimited_str'] + ")"
    elif ftype['lbm'] and ftype['bnd']:
        return "m_block->a_isBndryCell(" + occurrence['delimited_str'] + ")"
    else:
        return "a_isBndryCell(" + occurrence['delimited_str'] + ")"

def rewrite_gcell_inplace(occurrence, match, ftype):
    return "a_isBndryCellG(" + occurrence['delimited_str'] + ")"

# Rewrites ocurrences of custom collectors to a_isBndryCell(collector, ...)
def rewrite_custom_collector(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        if "cpu_cells" in match['begin_delimiter']:
            return "a_isBndryCell(cpu_cells, " + occurrence['delimited_str'] + ")"
        elif "input_cells" in match['begin_delimiter']:
            return "a_isBndryCell(input_cells, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
    else:
        if "cellsInput" in match['begin_delimiter']:
            return "a_isBndryCell(cellsInput, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
...
end_pId_re = "\]\s*\.m_isBndryCell"
end_pId_ggp_re = "\]\s*\.m_isBndryCell\)"
matchers = [
    # matches m_cells->a
...
tokens = [
            # # matches isBndryCell in grid files (shadowing)
            # {'pattern': r"\bisBndryCell\b",
            #  'replacement': "isBndryCell_",
            #  'ftype': ['grid'],
            # }
         ]
