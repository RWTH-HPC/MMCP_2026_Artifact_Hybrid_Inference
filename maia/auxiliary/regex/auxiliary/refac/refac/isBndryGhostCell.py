# Script to remove all accesses to the m_isBndryGhostCell member variable of the
# cell types inside ZFS.
#
# The following cases are identified (see also matchers below):
# - m_cells->a[ ... ].m_isBndryGhostCell
# - cells[ ... ].m_isBndryGhostCell
# - inside_cells->a[ ... ].m_isBndryGhostCell
# - cpu_cells->a[ ... ].m_isBndryGhostCell
#
...
# Rewrites occurrences inside grid to a_isBndryGhostCell(...), and inside blocks to
# a_isBndryGhostCell(...)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        return "a_isBndryGhostCell(" + occurrence['delimited_str'] + ")"
    elif ftype['fv'] and ftype['bnd']:
        return "m_block->a_isBndryGhostCell(" + occurrence['delimited_str'] + ")"
    else:
        return "a_isBndryGhostCell(" + occurrence['delimited_str'] + ")"
...
# Rewrites ocurrences of custom collectors to a_isBndryGhostCell(collector, ...)
def rewrite_custom_collector(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        if "cpu_cells" in match['begin_delimiter']:
            return "a_isBndryGhostCell(cpu_cells, " + occurrence['delimited_str'] + ")"
        elif "input_cells" in match['begin_delimiter']:
            return "a_isBndryGhostCell(input_cells, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
    else:
        if "cellsInput" in match['begin_delimiter']:
            return "a_isBndryGhostCell(cellsInput, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
...
end_pId_re = "\]\s*\.m_isBndryGhostCell\\b"
end_pId_ggp_re = "\]\s*\.m_isBndryGhostCell\)"
matchers = [
    # matches m_cells->a
...
tokens = [
            # # matches isBndryGhostCell in grid files (shadowing)
            # {'pattern': r"\bisBndryGhostCell\b",
            #  'replacement': "isBndryGhostCell_",
            #  'ftype': ['grid'],
            # }
         ]
