# Script to remove all accesses to the m_isGHaloCell member variable of the
# cell types inside ZFS.
#
# The following cases are identified (see also matchers below):
# - m_cells->a[ ... ].m_isGHaloCell
# - cells[ ... ].m_isGHaloCell
# - inside_cells->a[ ... ].m_isGHaloCell
# - cpu_cells->a[ ... ].m_isGHaloCell
#
...
# Rewrites occurrences inside grid to a_isGHaloCell(...), and inside blocks to
# a_isGHaloCell(...)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        return "a_isGHaloCell(" + occurrence['delimited_str'] + ")"
    else:
        return "a_isGHaloCell(" + occurrence['delimited_str'] + ")"

def rewrite_gcell_inplace(occurrence, match, ftype):
    return "a_isGHaloCellG(" + occurrence['delimited_str'] + ")"

# Rewrites ocurrences of custom collectors to a_isGHaloCell(collector, ...)
def rewrite_custom_collector(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        if "cpu_cells" in match['begin_delimiter']:
            return "a_isGHaloCell(cpu_cells, " + occurrence['delimited_str'] + ")"
        elif "input_cells" in match['begin_delimiter']:
            return "a_isGHaloCell(input_cells, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
    else:
        if "cellsInput" in match['begin_delimiter']:
            return "a_isGHaloCell(cellsInput, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
...
end_pId_re = "\]\s*\.m_isGHaloCell\\b"
end_pId_ggp_re = "\]\s*\.m_isGHaloCell\)"
matchers = [
    # matches m_cells->a
...
tokens = [
            # # matches isGHaloCell in grid files (shadowing)
            # {'pattern': r"\bisGHaloCell\b",
            #  'replacement': "isGHaloCell_",
            #  'ftype': ['grid'],
            # }
         ]
