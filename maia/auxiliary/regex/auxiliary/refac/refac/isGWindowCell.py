# Script to remove all accesses to the m_isGWindowCell member variable of the
# cell types inside ZFS.
#
# The following cases are identified (see also matchers below):
# - m_cells->a[ ... ].m_isGWindowCell
# - cells[ ... ].m_isGWindowCell
# - inside_cells->a[ ... ].m_isGWindowCell
# - cpu_cells->a[ ... ].m_isGWindowCell
#
...
# Rewrites occurrences inside grid to a_isGWindowCell(...), and inside blocks to
# a_isGWindowCell(...)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        return "a_isGWindowCell(" + occurrence['delimited_str'] + ")"
    else:
        return "a_isGWindowCell(" + occurrence['delimited_str'] + ")"

def rewrite_gcell_inplace(occurrence, match, ftype):
    return "a_isGWindowCellG(" + occurrence['delimited_str'] + ")"

# Rewrites ocurrences of custom collectors to a_isGWindowCell(collector, ...)
def rewrite_custom_collector(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        if "cpu_cells" in match['begin_delimiter']:
            return "a_isGWindowCell(cpu_cells, " + occurrence['delimited_str'] + ")"
        elif "input_cells" in match['begin_delimiter']:
            return "a_isGWindowCell(input_cells, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
    else:
        if "cellsInput" in match['begin_delimiter']:
            return "a_isGWindowCell(cellsInput, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
...
end_pId_re = "\]\s*\.m_isGWindowCell\\b"
end_pId_ggp_re = "\]\s*\.m_isGWindowCell\)"
matchers = [
    # matches m_cells->a
...
tokens = [
            # # matches isGWindowCell in grid files (shadowing)
            # {'pattern': r"\bisGWindowCell\b",
            #  'replacement': "isGWindowCell_",
            #  'ftype': ['grid'],
            # }
         ]
