# Script to remove all accesses to the m_isInterface member variable of the
# cell types inside ZFS.
#
# The following cases are identified (see also matchers below):
# - m_cells->a[ ... ].m_isInterface
# - cells[ ... ].m_isInterface
# - inside_cells->a[ ... ].m_isInterface
# - cpu_cells->a[ ... ].m_isInterface
#
...
# Rewrites occurrences inside grid to a_isInterface(...), and inside blocks to
# a_isInterface(...)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        return "a_isInterface(" + occurrence['delimited_str'] + ")"
    elif ftype['fv'] and ftype['bnd']:
        return "m_block->a_isInterface(" + occurrence['delimited_str'] + ")"
    else:
        return "a_isInterface(" + occurrence['delimited_str'] + ")"
...
# Rewrites ocurrences of custom collectors to a_isInterface(collector, ...)
def rewrite_custom_collector(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        if "cpu_cells" in match['begin_delimiter']:
            return "a_isInterface(cpu_cells, " + occurrence['delimited_str'] + ")"
        elif "input_cells" in match['begin_delimiter']:
            return "a_isInterface(input_cells, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
    else:
        if "cellsInput" in match['begin_delimiter']:
            return "a_isInterface(cellsInput, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
...
end_pId_re = "\]\s*\.m_isInterface\\b"
end_pId_ggp_re = "\]\s*\.m_isInterface\)"
matchers = [
    # matches m_cells->a
...
tokens = [
            # # matches isInterface in grid files (shadowing)
            # {'pattern': r"\bisInterface\b",
            #  'replacement': "isInterface_",
            #  'ftype': ['grid'],
            # }
         ]
