# Script to remove all accesses to the m_isInterfaceChild member variable of the
# cell types inside ZFS.
#
# The following cases are identified (see also matchers below):
# - m_cells->a[ ... ].m_isInterfaceChild
# - cells[ ... ].m_isInterfaceChild
# - inside_cells->a[ ... ].m_isInterfaceChild
# - cpu_cells->a[ ... ].m_isInterfaceChild
#
...
# Rewrites occurrences inside grid to a_isInterfaceChild(...), and inside blocks to
# a_isInterfaceChild(...)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        return "a_isInterfaceChild(" + occurrence['delimited_str'] + ")"
    elif ftype['interface']:
        return "m_block->a_isInterfaceChild(" + occurrence['delimited_str'] + ")"
    else:
        return "a_isInterfaceChild(" + occurrence['delimited_str'] + ")"

def rewrite_gcell_inplace(occurrence, match, ftype):
    return "a_isInterfaceChildG(" + occurrence['delimited_str'] + ")"

# Rewrites ocurrences of custom collectors to a_isInterfaceChild(collector, ...)
def rewrite_custom_collector(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        if "cpu_cells" in match['begin_delimiter']:
            return "a_isInterfaceChild(cpu_cells, " + occurrence['delimited_str'] + ")"
        elif "input_cells" in match['begin_delimiter']:
            return "a_isInterfaceChild(input_cells, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
    else:
        if "cellsInput" in match['begin_delimiter']:
            return "a_isInterfaceChild(cellsInput, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
...
end_pId_re = "\]\s*\.m_isInterfaceChild\\b"
end_pId_ggp_re = "\]\s*\.m_isInterfaceChild\)"
matchers = [
    # matches m_cells->a
...
tokens = [
            # # matches isInterfaceChild in grid files (shadowing)
            # {'pattern': r"\bisInterfaceChild\b",
            #  'replacement': "isInterfaceChild_",
            #  'ftype': ['grid'],
            # }
         ]
