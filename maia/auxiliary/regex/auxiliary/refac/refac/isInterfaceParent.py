# Script to remove all accesses to the m_isInterfaceParent member variable of the
# cell types inside ZFS.
#
# The following cases are identified (see also matchers below):
# - m_cells->a[ ... ].m_isInterfaceParent
# - cells[ ... ].m_isInterfaceParent
# - inside_cells->a[ ... ].m_isInterfaceParent
# - cpu_cells->a[ ... ].m_isInterfaceParent
#
...
# Rewrites occurrences inside grid to a_isInterfaceParent(...), and inside blocks to
# a_isInterfaceParent(...)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        return "a_isInterfaceParent(" + occurrence['delimited_str'] + ")"
    elif ftype['interface']:
        return "m_block->a_isInterfaceParent(" + occurrence['delimited_str'] + ")"
    else:
        return "a_isInterfaceParent(" + occurrence['delimited_str'] + ")"

def rewrite_gcell_inplace(occurrence, match, ftype):
    return "a_isInterfaceParentG(" + occurrence['delimited_str'] + ")"

# Rewrites ocurrences of custom collectors to a_isInterfaceParent(collector, ...)
def rewrite_custom_collector(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        if "cpu_cells" in match['begin_delimiter']:
            return "a_isInterfaceParent(cpu_cells, " + occurrence['delimited_str'] + ")"
        elif "input_cells" in match['begin_delimiter']:
            return "a_isInterfaceParent(input_cells, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
    else:
        if "cellsInput" in match['begin_delimiter']:
            return "a_isInterfaceParent(cellsInput, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
...
end_pId_re = "\]\s*\.m_isInterfaceParent\\b"
end_pId_ggp_re = "\]\s*\.m_isInterfaceParent\)"
matchers = [
    # matches m_cells->a
...
tokens = [
            # # matches isInterfaceParent in grid files (shadowing)
            # {'pattern': r"\bisInterfaceParent\b",
            #  'replacement': "isInterfaceParent_",
            #  'ftype': ['grid'],
            # }
         ]
