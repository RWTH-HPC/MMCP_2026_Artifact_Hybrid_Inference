# Script to remove all accesses to the m_kappa member variable of the
# cell types inside ZFS.
#
# The following cases are identified (see also matchers below):
# - m_cells->a[ ... ].m_kappa
# - cells[ ... ].m_kappa
# - inside_cells->a[ ... ].m_kappa
# - cpu_cells->a[ ... ].m_kappa
#
...
# Rewrites occurrences inside grid to a_kappa(...), and inside blocks to
# a_kappa(...)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        return "a_kappa(" + occurrence['delimited_str'] + ")"
    elif ftype['interface']:
        return "m_block->a_kappa(" + occurrence['delimited_str'] + ")"
    else:
        return "a_kappa(" + occurrence['delimited_str'] + ")"

def rewrite_gcell_inplace(occurrence, match, ftype):
    return "a_kappaG(" + occurrence['delimited_str'] + ")"

# Rewrites ocurrences of custom collectors to a_kappa(collector, ...)
def rewrite_custom_collector(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        if "cpu_cells" in match['begin_delimiter']:
            return "a_kappa(cpu_cells, " + occurrence['delimited_str'] + ")"
        elif "input_cells" in match['begin_delimiter']:
            return "a_kappa(input_cells, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
    else:
        if "cellsInput" in match['begin_delimiter']:
            return "a_kappa(cellsInput, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
...
end_pId_re = "\]\s*\.m_kappa\\b"
end_pId_ggp_re = "\]\s*\.m_kappa\)"
matchers = [
    # matches m_cells->a
...
tokens = [
            # # matches kappa in grid files (shadowing)
            # {'pattern': r"\bkappa\b",
            #  'replacement': "kappa_",
            #  'ftype': ['grid'],
            # }
