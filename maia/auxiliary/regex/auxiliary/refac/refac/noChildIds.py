# Script to remove all accesses to the m_noChildIds member variable of the
# cell types inside ZFS.
#
# The following cases are identified (see also matchers below):
# - m_cells->a[ ... ].m_noChildIds
# - cells[ ... ].m_noChildIds
# - inside_cells->a[ ... ].m_noChildIds
# - cpu_cells->a[ ... ].m_noChildIds
#
...
# Rewrites occurrences inside grid to a_noChildren(...), and inside blocks to
# grid().a_noChildren(...)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg'] or ftype['cell']:
        return "a_noChildren(" + occurrence['delimited_str'] + ")"
    else:
        return "grid().a_noChildren(" + occurrence['delimited_str'] + ")"

def rewrite_gcell_inplace(occurrence, match, ftype):
    return "a_noChildrenG(" + occurrence['delimited_str'] + ")"

def rewrite_pp_grid_inplace(occurrence, match, ftype):
    return "grid->a_noChildren(" + occurrence['delimited_str'] + ")"

# Rewrites ocurrences of custom collectors to a_noChildren(collector, ...)
def rewrite_custom_collector(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        if "cpu_cells" in match['begin_delimiter']:
            return "a_noChildren(cpu_cells, " + occurrence['delimited_str'] + ")"
        elif "input_cells" in match['begin_delimiter']:
            return "a_noChildren(input_cells, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
    else:
        if "cellsInput" in match['begin_delimiter']:
            return "grid().a_noChildren(cellsInput, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
...
end_pId_re = "\]\s*\.m_noChildIds"
end_pId_ggp_re = "\]\s*\.m_noChildIds\)"
end_noChildIdsG_re = "\]\s*\.m_noChildIds\[0\]"
matchers = [
    ...
    # matches input_cells (gCell version)
    {'begin_delimiter':"input_cells->a\s*\[",
     'end_delimiter':end_noChildIdsG_re,
     'rewrite_function':rewrite_custom_collector},
    # matches input_cells
    {'begin_delimiter':"input_cells->a\s*\[",
     'end_delimiter':end_pId_re,
...
tokens = [
            # # matches noChildIds in grid files (shadowing)
            # {'pattern': r"\bnoChildren\b",
            #  'replacement': "noChildren_",
            #  'ftype': ['grid'],
            # }
         ]
