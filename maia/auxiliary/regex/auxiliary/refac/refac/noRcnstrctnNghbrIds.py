# Script to remove all accesses to the m_noRcnstrctnNghbrIds member variable of the
# cell types inside ZFS.
#
# The following cases are identified (see also matchers below):
# - m_cells->a[ ... ].m_noRcnstrctnNghbrIds
# - cells[ ... ].m_noRcnstrctnNghbrIds
# - inside_cells->a[ ... ].m_noRcnstrctnNghbrIds
# - cpu_cells->a[ ... ].m_noRcnstrctnNghbrIds
#
...
# Rewrites occurrences inside blocks to :
# a_noReconstructionNeighbors(...)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['bnd']:
        if "block" in match['begin_delimiter']:
            return "m_block->a_noReconstructionNeighbors(" + occurrence['delimited_str'] + ")"
        else: 
            return "m_block->a_noReconstructionNeighbors(" + occurrence['delimited_str'] + ")"
    else:
        return "a_noReconstructionNeighbors(" + occurrence['delimited_str'] + ") "


def rewrite_gcell_inplace(occurrence, match, ftype):
    return "globalGId(" + occurrence['delimited_str'] + ")"

# Rewrites ocurrences of custom collectors to a_noReconstructionNeighbors(collector, ...)
def rewrite_custom_collector(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        if "cpu_cells" in match['begin_delimiter']:
            return "a_noReconstructionNeighbors(cpu_cells, " + occurrence['delimited_str'] + ")"
        elif "input_cells" in match['begin_delimiter']:
            return "a_noReconstructionNeighbors(input_cells, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
    else:
        if "cellsInput" in match['begin_delimiter']:
            return "grid().a_noReconstructionNeighbors(cellsInput, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
...
end_pId_re = "\]\s*\.m_noRcnstrctnNghbrIds\\b"
end_pId_ggp_re = "\]\s*\.m_noRcnstrctnNghbrIds\)"
matchers = [
...
tokens = [
            # # matches noRcnstrctnNghbrIds in grid files (shadowing)
            # {'pattern': r"\bnoRcnstrctnNghbrIds\b",
            #  'replacement': "noRcnstrctnNghbrIds_",
            #  'ftype': ['grid'],
            # }
         ]
