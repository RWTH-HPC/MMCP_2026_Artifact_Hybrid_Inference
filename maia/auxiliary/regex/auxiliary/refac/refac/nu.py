# Script to remove all accesses to the m_nu member variable of the
...
# Rewrites occurrences inside grid to a_nu(...), and inside blocks to
# a_nu(...)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        return "a_nu(" + occurrence['delimited_str'] + ")"
    elif ftype['lbm'] and (ftype['bnd'] or ftype['interface']):
        return "m_block->a_nu(" + occurrence['delimited_str'] + ")"
    else:
        return "a_nu(" + occurrence['delimited_str'] + ")"

def rewrite_gcell_inplace(occurrence, match, ftype):
    return "a_nuG(" + occurrence['delimited_str'] + ")"

# Rewrites ocurrences of custom collectors to a_nu(collector, ...)
def rewrite_custom_collector(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        if "cpu_cells" in match['begin_delimiter']:
            return "a_nu(cpu_cells, " + occurrence['delimited_str'] + ")"
        elif "input_cells" in match['begin_delimiter']:
            return "a_nu(input_cells, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
    else:
        if "cellsInput" in match['begin_delimiter']:
            return "a_nu(cellsInput, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
...
tokens = [
            # # matches nu in grid files (shadowing)
            # {'pattern': r"\bnu\b",
            #  'replacement': "nu_",
            #  'ftype': ['grid'],
            # }
