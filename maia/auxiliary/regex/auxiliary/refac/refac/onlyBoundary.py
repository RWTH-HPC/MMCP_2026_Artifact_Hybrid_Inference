# Script to remove all accesses to the m_onlyBoundary member variable of the
# cell types inside ZFS.
#
# The following cases are identified (see also matchers below):
# - m_cells->a[ ... ].m_onlyBoundary
# - cells[ ... ].m_onlyBoundary
# - inside_cells->a[ ... ].m_onlyBoundary
# - cpu_cells->a[ ... ].m_onlyBoundary
#
...
# Rewrites occurrences inside grid to a_onlyBoundary(...), and inside blocks to
# a_onlyBoundary(...)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        return "a_onlyBoundary(" + occurrence['delimited_str'] + ")"
    elif ftype['lbm'] and ftype['bnd']:
        return "m_block->a_onlyBoundary(" + occurrence['delimited_str'] + ")"
    else:
        return "a_onlyBoundary(" + occurrence['delimited_str'] + ")"
...
# Rewrites ocurrences of custom collectors to a_onlyBoundary(collector, ...)
def rewrite_custom_collector(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        if "cpu_cells" in match['begin_delimiter']:
            return "a_onlyBoundary(cpu_cells, " + occurrence['delimited_str'] + ")"
        elif "input_cells" in match['begin_delimiter']:
            return "a_onlyBoundary(input_cells, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
    else:
        if "cellsInput" in match['begin_delimiter']:
            return "a_onlyBoundary(cellsInput, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
...
end_pId_re = "\]\s*\.m_onlyBoundary\\b"
end_pId_ggp_re = "\]\s*\.m_onlyBoundary\)"
matchers = [
    # matches m_cells->a
...
tokens = [
            # # matches onlyBoundary in grid files (shadowing)
            # {'pattern': r"\bonlyBoundary\b",
            #  'replacement': "onlyBoundary_",
            #  'ftype': ['grid'],
            # }
         ]
