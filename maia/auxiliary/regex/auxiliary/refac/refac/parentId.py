# Script to remove all accesses to the m_parentId member variable of the
# cell types inside ZFS.
#
# The following cases are identified (see also matchers below):
# - m_cells->a[ ... ].m_parentId
# - cells[ ... ].m_parentId
# - inside_cells->a[ ... ].m_parentId
# - cpu_cells->a[ ... ].m_parentId
#
...
# Rewrites occurrences inside grid to a_parentId(...), and inside blocks to
# grid().a_parentId(...)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        return "a_parentId(" + occurrence['delimited_str'] + ")"
    else:
        return "grid().a_parentId(" + occurrence['delimited_str'] + ")"

def rewrite_gcell_inplace(occurrence, match, ftype):
    return "a_parentGId(" + occurrence['delimited_str'] + ")"

def rewrite_pp_grid_inplace(occurrence, match, ftype):
    return "grid->a_parentId(" + occurrence['delimited_str'] + ")"

# Rewrites ocurrences of custom collectors to a_parentId(collector, ...)
def rewrite_custom_collector(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        if "cpu_cells" in match['begin_delimiter']:
            return "a_parentId(cpu_cells, " + occurrence['delimited_str'] + ")"
        elif "input_cells" in match['begin_delimiter']:
            return "a_parentId(input_cells, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
    else:
        if "cellsInput" in match['begin_delimiter']:
            return "grid().a_parentId(cellsInput, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
...
end_pId_re = "\]\s*\.m_parentId"
end_pId_ggp_re = "\]\s*\.m_parentId\)"
matchers = [
