# Script to remove all accesses to the m_periodicDirection member variable of the
# cell types inside ZFS.
#
# The following cases are identified (see also matchers below):
# - m_cells->a[ ... ].m_periodicDirection
# - cells[ ... ].m_periodicDirection
# - inside_cells->a[ ... ].m_periodicDirection
# - cpu_cells->a[ ... ].m_periodicDirection
#
...
# Rewrites occurrences inside grid to a_periodicDirection(...), and inside blocks to
# a_periodicDirection(...)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        return "a_periodicDirection(" + occurrence['delimited_str'] + ")"
    elif ftype['fv'] and ftype['bnd']:
        return "m_block->a_periodicDirection(" + occurrence['delimited_str'] + ")"
    else:
        return "a_periodicDirection(" + occurrence['delimited_str'] + ")"

def rewrite_gcell_inplace(occurrence, match, ftype):
    return "a_periodicDirectionG(" + occurrence['delimited_str'] + ")"

# Rewrites ocurrences of custom collectors to a_periodicDirection(collector, ...)
def rewrite_custom_collector(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        if "cpu_cells" in match['begin_delimiter']:
            return "a_periodicDirection(cpu_cells, " + occurrence['delimited_str'] + ")"
        elif "input_cells" in match['begin_delimiter']:
            return "a_periodicDirection(input_cells, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
    else:
        if "cellsInput" in match['begin_delimiter']:
            return "a_periodicDirection(cellsInput, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
...
end_pId_re = "\]\s*\.m_periodicDirection\\b"
end_pId_ggp_re = "\]\s*\.m_periodicDirection\)"
matchers = [
    # matches m_cells->a
...
tokens = [
            # # matches periodicDirection in grid files (shadowing)
            # {'pattern': r"\bperiodicDirection\b",
            #  'replacement': "periodicDirection_",
            #  'ftype': ['grid'],
            # }
         ]
