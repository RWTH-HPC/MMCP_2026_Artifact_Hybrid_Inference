# Script to remove all accesses to the m_quasiPeriodicDirection member variable of the
# cell types inside ZFS.
#
# The following cases are identified (see also matchers below):
# - m_cells->a[ ... ].m_quasiPeriodicDirection
# - cells[ ... ].m_quasiPeriodicDirection
# - inside_cells->a[ ... ].m_quasiPeriodicDirection
# - cpu_cells->a[ ... ].m_quasiPeriodicDirection
#
...
# Rewrites occurrences inside grid to a_quasiPeriodicDirection(...), and inside blocks to
# a_quasiPeriodicDirection(...)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        return "a_quasiPeriodicDirection(" + occurrence['delimited_str'] + ")"
    elif ftype['fv'] and ftype['bnd']:
        return "m_block->a_quasiPeriodicDirection(" + occurrence['delimited_str'] + ")"
    else:
        return "a_quasiPeriodicDirection(" + occurrence['delimited_str'] + ")"

def rewrite_gcell_inplace(occurrence, match, ftype):
    return "a_quasiPeriodicDirectionG(" + occurrence['delimited_str'] + ")"

# Rewrites ocurrences of custom collectors to a_quasiPeriodicDirection(collector, ...)
def rewrite_custom_collector(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        if "cpu_cells" in match['begin_delimiter']:
            return "a_quasiPeriodicDirection(cpu_cells, " + occurrence['delimited_str'] + ")"
        elif "input_cells" in match['begin_delimiter']:
            return "a_quasiPeriodicDirection(input_cells, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
    else:
        if "cellsInput" in match['begin_delimiter']:
            return "a_quasiPeriodicDirection(cellsInput, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
...
end_pId_re = "\]\s*\.m_quasiPeriodicDirection\\b"
end_pId_ggp_re = "\]\s*\.m_quasiPeriodicDirection\)"
matchers = [
    # matches m_cells->a
...
tokens = [
            # # matches quasiPeriodicDirection in grid files (shadowing)
            # {'pattern': r"\bquasiPeriodicDirection\b",
            #  'replacement': "quasiPeriodicDirection_",
            #  'ftype': ['grid'],
            # }
         ]
