# Script to remove all accesses to the m_regridTrigger member variable of the
# cell types inside ZFS.
#
# The following cases are identified (see also matchers below):
# - m_cells->a[ ... ].m_regridTrigger
# - cells[ ... ].m_regridTrigger
#
...
# Rewrites occurrences inside grid to a_regridTrigger(...), and inside blocks to
# grid().a_regridTrigger(...)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        return "a_regridTrigger(" + occurrence['delimited_str'] + ")"
    else:
        return "grid().a_regridTrigger(" + occurrence['delimited_str'] + ")"

def rewrite_gcell_inplace(occurrence, match, ftype):
    return "a_regridTriggerG(" + occurrence['delimited_str'] + ")"

def rewrite_particle_inplace(occurrence, match, ftype):
    if "m_blockPtr" in match['begin_delimiter']:
        return "m_blockPtr->grid().a_regridTrigger(" + occurrence['delimited_str'] + ")"

def rewrite_gcell_inplace(occurrence, match, ftype):
    return "a_regridTriggerG(" + occurrence['delimited_str'] + ")"

def rewrite_pp_block_inplace(occurrence, match, ftype):
    return "ppblock()->a_regridTrigger(" + occurrence['delimited_str'] + ")"
...
def rewrite_custom_collector(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        if "cpu_cells" in match['begin_delimiter']:
            return "a_regridTrigger(cpu_cells, " + occurrence['delimited_str'] + ")"
        elif "input_cells" in match['begin_delimiter']:
            return "a_regridTrigger(input_cells, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
    else:
        if "cellsInput" in match['begin_delimiter']:
            return "grid().a_regridTrigger(cellsInput, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
...
end_regridTrigger_re = "\]\s*\.m_regridTrigger\s*\[\s*0\s*\]"
end_regridTrigger_re2 = "\]\s*\.m_regridTrigger\s*\[\s*j\s*\]"
end_regridTrigger_re3 = "\]\s*\.m_regridTrigger\s*\[\s*i\s*\]"
end_regridTrigger_re4 = "\]\s*\.m_regridTrigger\s*\[\s*dim\s*\]"
end_regridTrigger_re5 = "\]\s*\.m_regridTrigger\s*\[\s*spaceId\s*\]"
end_regridTrigger_re6 = "\]\s*\.m_regridTrigger\s*\[\s*k\s*\]"
end_regridTrigger_re7 = "\]\s*\.m_regridTrigger\s*\[\s*1\s*\]"
end_regridTrigger_re8 = "\]\s*\.m_regridTrigger\s*\[\s*d\s*\]"
end_regridTrigger_re9 = "\]\s*\.m_regridTrigger\s*\[\s*spaceID\s*\]"
end_regridTrigger_re10 = "\]\s*\.m_regridTrigger\s*\[\s*spaceId1\s*\]"
end_regridTrigger_re11 = "\]\s*\.m_regridTrigger\s*\[\s*2\s*\]"
end_regridTrigger_re12 = "\]\s*\.m_regridTrigger\s*"
matchers = [
    # matches ppblock()->m_cells->a
    {'begin_delimiter':"ppblock()->m_cells->a\s*\[",
     'end_delimiter':end_regridTrigger_re,
    ...
    # matches m_cells->a
    {'begin_delimiter':"m_cells->a\s*\[",
     'end_delimiter':end_regridTrigger_re2,
     'rewrite_function':rewrite_inplace},
    {'begin_delimiter':"cells\s*\[",
     'end_delimiter':end_regridTrigger_re2,
     'rewrite_function':rewrite_inplace},
    # matches m_cells->a
    {'begin_delimiter':"m_cells->a\s*\[",
     'end_delimiter':end_regridTrigger_re3,
     'rewrite_function':rewrite_inplace},
    # matches m_cells->a
    {'begin_delimiter':"cells\s*\[",
     'end_delimiter':end_regridTrigger_re3,
    ...
    # matches m_cells->a
    {'begin_delimiter':"m_cells->a\s*\[",
     'end_delimiter':end_regridTrigger_re4,
     'rewrite_function':rewrite_inplace},
    # matches m_cells->a
    {'begin_delimiter':"cells\s*\[",
     'end_delimiter':end_regridTrigger_re4,
    ...
    # matches m_cells->a
    {'begin_delimiter':"cells\s*\[",
     'end_delimiter':end_regridTrigger_re5,
     'rewrite_function':rewrite_inplace},
    # matches m_cells->a
    {'begin_delimiter':"cells\s*\[",
     'end_delimiter':end_regridTrigger_re6,
     'rewrite_function':rewrite_inplace},
    # matches m_cells->a
    {'begin_delimiter':"m_cells->a\s*\[",
     'end_delimiter':end_regridTrigger_re6,
    ...
    # matches m_cells->a
    {'begin_delimiter':"cells\s*\[",
     'end_delimiter':end_regridTrigger_re7,
     'rewrite_function':rewrite_inplace},
    # matches m_cells->a
    {'begin_delimiter':"m_cells->a\s*\[",
     'end_delimiter':end_regridTrigger_re7,
    ...
    {'begin_delimiter':"\*\(m_blockPtr->cells\s*\[",
     'end_delimiter':"\]\s*\.m_regridTrigger\s*\)",
    ...
    {'begin_delimiter':"\*\(m_pCells\s*\[",
     ...
     'rewrite_function':rewrite_inplace},
    {'begin_delimiter':"cells\s*\[",
     'end_delimiter':end_regridTrigger_re8,
     'rewrite_function':rewrite_inplace},
    {'begin_delimiter':"cells\s*\[",
     'end_delimiter':end_regridTrigger_re9,
     'rewrite_function':rewrite_inplace},
    {'begin_delimiter':"cells\s*\[",
     'end_delimiter':end_regridTrigger_re10,
     'rewrite_function':rewrite_inplace},
    {'begin_delimiter':"m_cells->a\s*\[",
     'end_delimiter':end_regridTrigger_re11,
     'rewrite_function':rewrite_inplace},
    ...
    {'begin_delimiter':"m_gCells->a\s*\[",
     'end_delimiter':end_regridTrigger_re12,
