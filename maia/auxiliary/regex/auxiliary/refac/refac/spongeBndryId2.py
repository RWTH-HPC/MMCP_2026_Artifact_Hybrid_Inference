# Script to remove all accesses to the m_spongeBndryId2 member variable of the
# cell types inside ZFS.
#
# The following cases are identified (see also matchers below):
# - m_cells->a[ ... ].m_spongeBndryId2
# - cells[ ... ].m_spongeBndryId2
# - inside_cells->a[ ... ].m_spongeBndryId2
# - cpu_cells->a[ ... ].m_spongeBndryId2
#
...
# Rewrites occurrences inside grid to a_spongeBndryId(..., 1), and inside blocks to
# grid().a_spongeBndryId(..., 1)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['bnd']:
        if "block" in match['begin_delimiter']:
            return "m_block->a_spongeBndryId(" + occurrence['delimited_str'] + ", 1)"
        else: 
            return "m_block->a_spongeBndryId(" + occurrence['delimited_str'] + ", 1)"
    else:
        return "a_spongeBndryId(" + occurrence['delimited_str'] + ", 1)"


def rewrite_gcell_inplace(occurrence, match, ftype):
    return "globalGId(" + occurrence['delimited_str'] + ")"

# Rewrites ocurrences of custom collectors to a_spongeBndryId(collector, ..., 1)
def rewrite_custom_collector(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        if "cpu_cells" in match['begin_delimiter']:
            return "a_spongeBndryId(cpu_cells, " + occurrence['delimited_str'] + ", 1)"
        elif "input_cells" in match['begin_delimiter']:
            return "a_spongeBndryId(input_cells, " + occurrence['delimited_str'] + ", 1)"
        else:
            exit("unknown pattern")
    else:
        if "cellsInput" in match['begin_delimiter']:
            return "grid().a_spongeBndryId(cellsInput, " + occurrence['delimited_str'] + ", 1)"
        else:
            exit("unknown pattern")
...
end_pId_re = "\]\s*\.m_spongeBndryId2\\b"
end_pId_ggp_re = "\]\s*\.m_spongeBndryId2\)"
matchers = [
...
tokens = [
            # # matches spongeBndryId2 in grid files (shadowing)
            # {'pattern': r"\bspongeBndryId2\b",
            #  'replacement': "spongeBndryId2_",
            #  'ftype': ['grid'],
            # }
         ]
