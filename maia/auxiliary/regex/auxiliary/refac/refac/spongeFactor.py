# Script to remove all accesses to the m_spongeFactor member variable of the
# cell types inside ZFS.
#
# The following cases are identified (see also matchers below):
# - m_cells->a[ ... ].m_spongeFactor
# - cells[ ... ].m_spongeFactor
# - inside_cells->a[ ... ].m_spongeFactor
# - cpu_cells->a[ ... ].m_spongeFactor
#
...
# Rewrites occurrences inside grid to a_spongeFactor(...), and inside blocks to
# grid().a_spongeFactor(...)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['bnd']:
        if "block" in match['begin_delimiter']:
            return "m_block->a_spongeFactor(" + occurrence['delimited_str'] + ")"
        else: 
            return "m_block->a_spongeFactor(" + occurrence['delimited_str'] + ")"
    else:
        return "a_spongeFactor(" + occurrence['delimited_str'] + ") "


def rewrite_gcell_inplace(occurrence, match, ftype):
    return "globalGId(" + occurrence['delimited_str'] + ")"

# Rewrites ocurrences of custom collectors to a_spongeFactor(collector, ...)
def rewrite_custom_collector(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        if "cpu_cells" in match['begin_delimiter']:
            return "a_spongeFactor(cpu_cells, " + occurrence['delimited_str'] + ")"
        elif "input_cells" in match['begin_delimiter']:
            return "a_spongeFactor(input_cells, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
    else:
        if "cellsInput" in match['begin_delimiter']:
            return "grid().a_spongeFactor(cellsInput, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
...
end_pId_re = "\]\s*\.m_spongeFactor\\b"
end_pId_ggp_re = "\]\s*\.m_spongeFactor\)"
matchers = [
...
tokens = [
            # # matches spongeFactor in grid files (shadowing)
            # {'pattern': r"\bspongeFactor\b",
            #  'replacement': "spongeFactor_",
            #  'ftype': ['grid'],
            # }
         ]
