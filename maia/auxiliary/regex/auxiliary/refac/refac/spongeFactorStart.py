# Script to remove all accesses to the m_spongeFactorStart member variable of the
# cell types inside ZFS.
#
# The following cases are identified (see also matchers below):
# - m_cells->a[ ... ].m_spongeFactorStart
# - cells[ ... ].m_spongeFactorStart
# - inside_cells->a[ ... ].m_spongeFactorStart
# - cpu_cells->a[ ... ].m_spongeFactorStart
#
...
# Rewrites occurrences inside grid to a_spongeFactorStart(...), and inside blocks to
# grid().a_spongeFactorStart(...)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['bnd']:
        if "block" in match['begin_delimiter']:
            return "m_block->a_spongeFactorStart(" + occurrence['delimited_str'] + ")"
        else: 
            return "m_block->a_spongeFactorStart(" + occurrence['delimited_str'] + ")"
    else:
        return "a_spongeFactorStart(" + occurrence['delimited_str'] + ") "


def rewrite_gcell_inplace(occurrence, match, ftype):
    return "globalGId(" + occurrence['delimited_str'] + ")"

# Rewrites ocurrences of custom collectors to a_spongeFactorStart(collector, ...)
def rewrite_custom_collector(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        if "cpu_cells" in match['begin_delimiter']:
            return "a_spongeFactorStart(cpu_cells, " + occurrence['delimited_str'] + ")"
        elif "input_cells" in match['begin_delimiter']:
            return "a_spongeFactorStart(input_cells, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
    else:
        if "cellsInput" in match['begin_delimiter']:
            return "grid().a_spongeFactorStart(cellsInput, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
...
end_pId_re = "\]\s*\.m_spongeFactorStart\\b"
end_pId_ggp_re = "\]\s*\.m_spongeFactorStart\)"
matchers = [
...
tokens = [
            # # matches spongeFactorStart in grid files (shadowing)
            # {'pattern': r"\bspongeFactorStart\b",
            #  'replacement': "spongeFactorStart_",
            #  'ftype': ['grid'],
            # }
         ]
