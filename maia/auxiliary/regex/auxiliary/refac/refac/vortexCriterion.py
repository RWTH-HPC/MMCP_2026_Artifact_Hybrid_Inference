# Script to remove all accesses to the m_vortexCriterion member variable of the
# cell types inside ZFS.
#
# The following cases are identified (see also matchers below):
# - m_cells->a[ ... ].m_vortexCriterion
# - cells[ ... ].m_vortexCriterion
# - inside_cells->a[ ... ].m_vortexCriterion
# - cpu_cells->a[ ... ].m_vortexCriterion
#
...
# Rewrites occurrences inside grid to a_vortexCriterion(...), and inside blocks to
# a_vortexCriterion(...)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        return "a_vortexCriterion(" + occurrence['delimited_str'] + ")"
    else:
        return "a_vortexCriterion(" + occurrence['delimited_str'] + ")"

def rewrite_gcell_inplace(occurrence, match, ftype):
    return "a_vortexCriterionG(" + occurrence['delimited_str'] + ")"

# Rewrites ocurrences of custom collectors to a_vortexCriterion(collector, ...)
def rewrite_custom_collector(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        if "cpu_cells" in match['begin_delimiter']:
            return "a_vortexCriterion(cpu_cells, " + occurrence['delimited_str'] + ")"
        elif "input_cells" in match['begin_delimiter']:
            return "a_vortexCriterion(input_cells, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
    else:
        if "cellsInput" in match['begin_delimiter']:
            return "a_vortexCriterion(cellsInput, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
...
end_pId_re = "\]\s*\.m_vortexCriterion\\b"
end_pId_ggp_re = "\]\s*\.m_vortexCriterion\)"
matchers = [
    # matches m_cells->a
...
tokens = [
            # # matches vortexCriterion in grid files (shadowing)
            # {'pattern': r"\bvortexCriterion\b",
            #  'replacement': "vortexCriterion_",
            #  'ftype': ['grid'],
            # }
         ]
