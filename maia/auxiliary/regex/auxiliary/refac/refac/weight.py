# Script to remove all accesses to the m_weight member variable of the
# cell types inside ZFS.
#
# The following cases are identified (see also matchers below):
# - m_cells->a[ ... ].m_weight
# - cells[ ... ].m_weight
# - inside_cells->a[ ... ].m_weight
# - cpu_cells->a[ ... ].m_weight
#
...
# Rewrites occurrences inside grid to a_weight(...), and inside blocks to
# grid().a_weight(...)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        return "a_weight(" + occurrence['delimited_str'] + ")"
    else:
        return "grid().a_weight(" + occurrence['delimited_str'] + ")"

def rewrite_gcell_inplace(occurrence, match, ftype):
    return "a_weightG(" + occurrence['delimited_str'] + ")"

# Rewrites ocurrences of custom collectors to a_weight(collector, ...)
def rewrite_custom_collector(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        if "cpu_cells" in match['begin_delimiter']:
            return "a_weight(cpu_cells, " + occurrence['delimited_str'] + ")"
        elif "input_cells" in match['begin_delimiter']:
            return "a_weight(input_cells, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
    else:
        if "cellsInput" in match['begin_delimiter']:
            return "grid().a_weight(cellsInput, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
...
end_pId_re = "\]\s*\.m_weight"
end_pId_ggp_re = "\]\s*\.m_weight\)"
matchers = [
    # matches m_cells->a
...
tokens = [
            # # matches weight in grid files (shadowing)
            # {'pattern': r"\bweight\b",
            #  'replacement': "weight_",
            #  'ftype': ['grid'],
            # }
