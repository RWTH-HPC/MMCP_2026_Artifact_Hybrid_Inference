# Script to remove all accesses to the m_workLoad member variable of the
# cell types inside ZFS.
#
# The following cases are identified (see also matchers below):
# - m_cells->a[ ... ].m_workLoad
# - cells[ ... ].m_workLoad
# - inside_cells->a[ ... ].m_workLoad
# - cpu_cells->a[ ... ].m_workLoad
#
...
# Rewrites occurrences inside grid to a_workLoad(...), and inside blocks to
# a_workLoad(...)
def rewrite_inplace(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        return "a_workLoad(" + occurrence['delimited_str'] + ")"
    else:
        return "a_workLoad(" + occurrence['delimited_str'] + ")"

def rewrite_gcell_inplace(occurrence, match, ftype):
    return "a_workLoadG(" + occurrence['delimited_str'] + ")"

# Rewrites ocurrences of custom collectors to a_workLoad(collector, ...)
def rewrite_custom_collector(occurrence, match, ftype):
    if ftype['grid'] or ftype['avg']:
        if "cpu_cells" in match['begin_delimiter']:
            return "a_workLoad(cpu_cells, " + occurrence['delimited_str'] + ")"
        elif "input_cells" in match['begin_delimiter']:
            return "a_workLoad(input_cells, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
    else:
        if "cellsInput" in match['begin_delimiter']:
            return "a_workLoad(cellsInput, " + occurrence['delimited_str'] + ")"
        else:
            exit("unknown pattern")
...
end_pId_re = "\]\s*\.m_workLoad\\b"
end_pId_ggp_re = "\]\s*\.m_workLoad\)"
matchers = [
    # matches m_cells->a
...
tokens = [
            # # matches workLoad in grid files (shadowing)
            # {'pattern': r"\bworkLoad\b",
            #  'replacement': "workLoad_",
            #  'ftype': ['grid'],
            # }
         ]
