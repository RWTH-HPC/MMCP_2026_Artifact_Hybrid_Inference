...
# Special case for zfslbmblockdxqy.cpp
SUFFIX="\.[${NEWLINE} ${TAB}]*${OLDNAME}[${NEWLINE} ${TAB}]*"
PREFIX="${PREF1}"
REPLACE="variableptr(\2)"
if ftype zfslbmblockdxqy || ftype zfslbmblockd2q9; then
  CONTENT=$(regex "$CONTENT" "$PREFIX" "$SUFFIX" "$REPLACE")
fi

#Special case for zfspostprocessingblock.cpp
SUFFIX="\.[${NEWLINE} ${TAB}]*${OLDNAME}[${NEWLINE} ${TAB}]*"
PREFIX="ppblock()->${PREF1}"
REPLACE="\&ppblock()->a_variable(\2, 0)"
if ftype zfspostprocessingblock; then
  CONTENT=$(regex "$CONTENT" "$PREFIX" "$SUFFIX" "$REPLACE")
fi

#Special case for zfslbmblock.cpp
SUFFIX="\.[${NEWLINE} ${TAB}]*${OLDNAME}[${NEWLINE} ${TAB}]*"
PREFIX="grid().${PREF1}"
REPLACE="\&grid().a_variable(\2, 0)"
if ftype zfslbmblock; then
  CONTENT=$(regex "$CONTENT" "$PREFIX" "$SUFFIX" "$REPLACE")
fi
