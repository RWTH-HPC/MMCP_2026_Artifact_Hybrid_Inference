...
#ifndef PARALLELIO_PNETCDF_H
#define PARALLELIO_PNETCDF_H

#include <map>

class ParallelIoPNetcdf : public ParallelIoBase<ParallelIoPNetcdf> {
 ...
 public:
  ...
  // Parallel Netcdf-specific methods (private)
  // -> only needed inside the class (rationale: the actual data I/O calls
 ...
 private:
  ...
  // Data mode methods
  template <class T>
  void b_writeArray(const T* const array, const MString& name, const size_type noDims, MPI_Offset* start,
                    MPI_Offset* count, MPI_Offset memoryStride, const size_type noChunks, MPI_Offset diskStride);
  template <class T>
  ...
  template <class T>
  void b_readArray(T* array, const MString& name, const size_type noDims, MPI_Offset* start, MPI_Offset* count,
  ...
  template <class T>
  void b_getAttribute(T* const value, const MString& name, const MString& datasetName = "",
                      const size_type totalCount = 1);
  ...
  // Parallel Netcdf-specific members
 private:
  MInt b_ncId = -1;
